...
public:
  ...
  // ---- SA-LIVO diagnostic: original FAST-LIVO2 VIO as an information-form
  //      front-end (tracking/Jacobians from VIOManager, fusion via SAIF) ----
  bool info_form_only_ = false;
  Eigen::Matrix<double, 6, 6> vio_Lambda_;
  Eigen::Matrix<double, 6, 1> vio_b_;
  void beginInfoForm();
  void endInfoForm();
  const Eigen::Matrix<double, 6, 6> &vioLambda() const { return vio_Lambda_; }
  const Eigen::Matrix<double, 6, 1> &viob() const { return vio_b_; }
  void prepareVisualInfoForm(cv::Mat &img, vector<pointWithVar> &pg,
                             const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &feat_map,
                             double img_time);
  void finalizeVisualFrame(cv::Mat &img, vector<pointWithVar> &pg,
                           const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &feat_map);
