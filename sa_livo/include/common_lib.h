...
// SA-LIVO Sect. IV adopts the hybrid RIGHT-invariant error convention
// (R = Exp(delta_phi) R_hat, attitude self-transition = I), whereas the
// FAST-LIVO2 base uses the LEFT-invariant convention (R = R_hat Exp(delta_phi)).
// This flag switches the retraction, the IMU error propagation and the
// LiDAR/visual Jacobians consistently.  Default false keeps the proven
// FAST-LIVO2 behaviour; the paper-faithful path is enabled per run.
inline bool g_right_invariant_en = false;
...
struct StatesGroup
{
  ...
  {
    StatesGroup a;
    const M3D dR = Exp(state_add(0, 0), state_add(1, 0), state_add(2, 0));
    if (g_right_invariant_en) a.rot_end = dR * this->rot_end;
    else a.rot_end = this->rot_end * dR;
  ...
  {
    const M3D dR = Exp(state_add(0, 0), state_add(1, 0), state_add(2, 0));
    if (g_right_invariant_en) this->rot_end = dR * this->rot_end;
    else this->rot_end = this->rot_end * dR;
  ...
  Matrix<double, DIM_STATE, 1> operator-(const StatesGroup &b)
  {
    ...
    M3D rotd;
    if (g_right_invariant_en) rotd = this->rot_end * b.rot_end.transpose();
    else rotd = b.rot_end.transpose() * this->rot_end;
