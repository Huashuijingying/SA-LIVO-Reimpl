...
void ImuProcess::cache_init_imu(const sensor_msgs::msg::Imu::ConstSharedPtr &imu)
{
  if (imu_need_init && static_cast<int>(init_imu_samples_.size()) < MAX_INI_COUNT)
    init_imu_samples_.push_back(imu);
}

void ImuProcess::IMU_init(const MeasureGroup &meas, StatesGroup &state_inout, int &N)
{
  if (use_baseline_init_)
  {
    // Original FAST-LIVO2 initialization: first frame seeds the running mean,
    // then every IMU in the current measure group is folded in with 1/N.
    if (b_first_frame)
    {
      Reset();
      N = 1;
  ...
  // Use the deterministic callback-order prefix, but only samples whose stamp
  // is not later than this measurement target. This makes initialization
  // independent of how spin_some() batches messages.
  const double target_time = std::max(meas.lio_time, meas.vio_time);
  int count = 0;
  mean_acc.setZero();
  mean_gyr.setZero();
  for (const auto &imu : init_imu_samples_)
  {
    if (count >= MAX_INI_COUNT || stamp2Sec(imu->header.stamp) > target_time + 1e-9) break;
    mean_acc += V3D(imu->linear_acceleration.x, imu->linear_acceleration.y, imu->linear_acceleration.z);
    mean_gyr += V3D(imu->angular_velocity.x, imu->angular_velocity.y, imu->angular_velocity.z);
    ++count;
  }
  N = count;
  ...
  mean_acc /= static_cast<double>(count);
  mean_gyr /= static_cast<double>(count);
  IMU_mean_acc_norm = mean_acc.norm();
  state_inout.gravity = -mean_acc / mean_acc.norm() * G_m_s2;
  state_inout.rot_end = Eye3d;
  state_inout.bias_g = Zero3d;
  b_first_frame = false;
  if (!meas.imu.empty()) last_imu = meas.imu.back();
}
...
void ImuProcess::UndistortPcl(LidarMeasureGroup &lidar_meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  // The LIO preprocessing event owns IMU backward undistortion. The following
  // VIO event reuses this immutable camera-time cloud for the single joint
  // update and must not compensate it a second time.
  ...
  case VIO:
    ...
    for (int i = 0; i < v_imu.size() - 1; i++)
    {
      ...
      // SA-LIVO Sect. IV (right-invariant): the attitude self-transition is
      // identity and the velocity-rotation coupling uses the world-centred
      // specific-force skew.  FAST-LIVO2 (left-invariant) rotates the
      // attitude error by Exp(-omega*dt) and uses the body-frame skew.
      if (g_right_invariant_en)
      {
        F_x.block<3, 3>(0, 0) = Eye3d;
        M3D acc_world_skew;
        // SKEW_SYM_MATRX(v) textually expands -v[2], so it must receive a
        // simple vector, not an expression (R_imu * acc_avr would expand to
        // -R_imu * acc_avr[2], i.e. matrix * scalar).
        const V3D acc_world = R_imu * acc_avr;
        acc_world_skew << SKEW_SYM_MATRX(acc_world);
        F_x.block<3, 3>(7, 0) = -acc_world_skew * dt;
        if (ba_bg_est_en) F_x.block<3, 3>(0, 10) = -R_imu * dt;
      }
      else
      {
      ...
      if (g_right_invariant_en)
        cov_w.block<3, 3>(0, 0) =
            R_imu * cov_gyr.asDiagonal() * R_imu.transpose() * dt * dt;
      else
  ...
  /*** undistort each lidar point during the LIO preprocessing event ***/
  if (lidar_meas.lio_vio_flg == LIO)
  {
...
void ImuProcess::Process2(LidarMeasureGroup &lidar_meas, StatesGroup &stat, PointCloudXYZI::Ptr cur_pcl_un_)
{
  ...
  if (imu_need_init)
  {
    ...
    // Initialization frames are not mapping frames, but their target time
    // must still advance. Otherwise the first normal update integrates an
    // initialization-length LiDAR segment whose duration depends on callback
    // batching.
    lidar_meas.last_lio_update_time = pcl_end_time;
