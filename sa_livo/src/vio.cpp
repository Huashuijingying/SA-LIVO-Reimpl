...
void VIOManager::beginInfoForm()
{
  info_form_only_ = true;
  vio_Lambda_.setZero();
  vio_b_.setZero();
}

void VIOManager::endInfoForm()
{
  info_form_only_ = false;
}
...
void VIOManager::computeJacobianAndUpdateEKF(cv::Mat img)
{
  ...
  if (info_form_only_)
  {
    // Information-form front-end: evaluate the photometric residuals once at
    // the propagated state (finest pyramid level) and accumulate ΛV, bV.
    if (inverse_composition_en) updateStateInverse(img, 0);
    else updateState(img, 0);
    return;
  }

  for (int level = patch_pyrimid_level - 1; level >= 0; level--)
  {
    if (inverse_composition_en) { has_ref_patch_cache = false; updateStateInverse(img, level); }
...
void VIOManager::updateStateInverse(cv::Mat img, int level)
{
  ...
  for (int iteration = 0; iteration < max_iterations; iteration++)
  {
    ...
    if (info_form_only_)
    {
      vio_Lambda_ += H_sub.transpose() * H_sub;
      vio_b_ -= H_sub.transpose() * z;
      return;
    }
...
void VIOManager::updateState(cv::Mat img, int level)
{
  ...
  for (int iteration = 0; iteration < max_iterations; iteration++)
  {
    ...
    if (info_form_only_)
    {
      vio_Lambda_ += H_sub.leftCols(6).transpose() * H_sub.leftCols(6);
      vio_b_ -= H_sub.leftCols(6).transpose() * z;
      return;
    }
...
void VIOManager::processFrame(cv::Mat &img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &feat_map, double img_time)
{
  if (width != img.cols || height != img.rows)
  {
    ...
    cv::Mat resized;
    cv::resize(img, resized, cv::Size(img.cols * image_resize_factor, img.rows * image_resize_factor), 0, 0, CV_INTER_LINEAR);
    img = resized;
  }
  img_rgb = img.clone();
  img_cp = img.clone();
...
void VIOManager::prepareVisualInfoForm(cv::Mat &img, vector<pointWithVar> &pg,
                                       ...
                                       double img_time)
{
  ...
  resetGrid();
  retrieveFromVisualSparseMap(img, pg, feat_map);
  computeJacobianAndUpdateEKF(img);  // info_form_only_ active: no state mutation
}

void VIOManager::finalizeVisualFrame(cv::Mat &img, vector<pointWithVar> &pg,
                                     const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &feat_map)
{
  if (width != img.cols || height != img.rows)
  {
